...
export async function getCountriesByRegion(region: string): Promise<Country[]> {
    ...
    if (region === 'asia-oceania') {
        const asia = await fetchCountries('https://restcountries.com/v3.1/region/asia');
        const oceania = await fetchCountries('https://restcountries.com/v3.1/region/oceania');
        return [...asia, ...oceania];
    }
    
    const validRegions = ['africa', 'americas', 'europe'];
    if (validRegions.includes(region)) {
        return await fetchCountries(`https://restcountries.com/v3.1/region/${region}`);
    }
